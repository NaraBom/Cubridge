...
import { Cube, MealPlan, MEAL_TIMES } from '@/types';
import { getCubes, getLogs, getMealPlans, upsertMealPlan, addLog, deleteLog, markMealPlansLogged } from '@/lib/storage';
          ...
          {cubes.map((cube) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 text-sm font-medium text-gray-700">{cube.name}</span>
          ...
          <thead>
            <tr>
              ...
              {weekDates.map((date, i) => {
                ...
                return (
                  <th key={key} className="pb-2 text-center">
                    <div className={`inline-flex flex-col items-center gap-0.5`}>
                      <span className="text-xs text-gray-400">{DAY_LABELS[i]}</span>
                      <span className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${
                        isToday ? 'bg-[var(--primary)] text-white' : 'text-gray-700'
                      }`}>
                        {date.getDate()}
                      </span>
                    </div>
              ...
              <tr key={mealTime} className="align-top">
                ...
                {weekDates.map((date) => {
                  ...
                  const plannedEntries = Object.entries(cubeCounts).map(([id, count]) => ({
                    ...
                    count,
                  })).filter((e) => e.cube) as { cube: Cube; count: number }[];
                  const isPast = date < today && !isSameDay(date, today);

                  return (
                    <td key={dateKey} className="p-1 align-top">
                      <div className={`min-h-[72px] rounded-xl border p-1.5 flex flex-col gap-1 transition-colors ${
                        ...
                        {plannedEntries.map(({ cube, count }) => (
                          <div
                            key={cube.id}
                            className={`flex items-center gap-1 text-xs px-1.5 py-0.5 rounded-lg border ${MEAL_COLORS[mealTime]} ${isPast ? 'opacity-50' : ''}`}
                          >
                            <span className="leading-none">{cube.emoji}</span>
                            <span className="truncate flex-1 font-medium" style={{ maxWidth: '48px' }}>{cube.name}</span>
